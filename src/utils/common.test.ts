...
Deno.test("[depthFirstTraversal] traversal a tree", () => {
  let tree = {
    value: "a1",
    children: [{ value: "b1", children: [{ value: "c1" }] }, { value: "b2" }],
  };
  let items: Tree[] = [];
  ...
  asserts.assertEquals(
    items,
    [tree, tree.children[0], tree.children[0].children?.[0], tree.children[1]],
  );
});
Deno.test("[depthFirstTraversal] traversal a tree[]", () => {
  ...
  asserts.assertEquals(
    items,
    [tree1, tree1.children[0], tree1.children[1], tree2],
  );
});
